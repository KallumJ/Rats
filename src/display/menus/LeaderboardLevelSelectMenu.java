...
public class LeaderboardLevelSelectMenu extends GameMenu {
    ...
    public Parent buildMenu() {
        ...
        for (Player allPlayer : allPlayers) {
            allPlayer.getHighScores().forEach((level, score) ->
                    levelsWithScore.add(level));
        }

        // Create a menu item for each level
        for (Integer level : levelsWithScore) {
            LeaderboardLevelMenuItem menuItem = new LeaderboardLevelMenuItem(String.valueOf(level));
